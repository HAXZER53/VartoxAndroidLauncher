...
public class MinecraftAccount {
    ...
    void updateSkinFace(String uuid) {
        try {
            ...
            String vartoxSkinUrl = net.kdt.pojavlaunch.vartox.VartoxConfig.SKIN_API_URL.replace("%username%", username);
            try {
                Tools.downloadFile(vartoxSkinUrl, skinFile.getAbsolutePath());
                Log.i("SkinLoader", "Update skin face success from Vartox API");
                return;
            } catch (Exception skinEx) {
                Log.w("SkinLoader", "Vartox skin API failed, trying mc-heads fallback", skinEx);
            }
            Tools.downloadFile("https://mc-heads.net/head/" + uuid + "/100", skinFile.getAbsolutePath());
            Log.i("SkinLoader", "Update skin face success from fallback");
        } catch (IOException e) {
